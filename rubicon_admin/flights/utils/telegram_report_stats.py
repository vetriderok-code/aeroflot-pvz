...
_FLIGHT_NUM_TD_RE = re.compile(r'(?i)вылет\s*[:\s]+(\d+)\b')
_DATE_RE = re.compile(r'\b(\d{1,2}\.\d{1,2}(?:\.\d{2,4})?)\b')
_CALLSIGN_RE = re.compile(r'(?i)позывной\s*[:\s]+([^\n\r]+)')
_PILOT_RE = re.compile(r'(?i)пилот\s*[:\s]+([^\n\r]+)')
_OPERATOR_RE = re.compile(r'(?i)оператор\s*[:\s]+([^\n\r]+)')
_SK42_VALUE_RE = re.compile(r'(?i)[xyхyuю]\s*[=:]\s*(\d{6,8})')
_SK42_X_RE = re.compile(r'(?i)(?:^|\s)[xх]\s*[=:]\s*(\d{6,8})')
_SK42_Y_RE = re.compile(r'(?i)(?:^|\s)[yую]\s*[=:]\s*(\d{6,8})')
_COORDS_LINE_RE = re.compile(
    r'(?i)координат\w*\s*[:\-]?\s*(\d{6,8})\s*[,;\s]\s*(\d{6,8})'
)
_UKRAINE_LAT = (44.0, 52.5)
_UKRAINE_LON = (22.0, 42.0)
...
def _in_ukraine_bbox(lat: float, lon: float) -> bool:
    return (
        _UKRAINE_LAT[0] <= lat <= _UKRAINE_LAT[1]
        and _UKRAINE_LON[0] <= lon <= _UKRAINE_LON[1]
    )


def _sk42_pair_to_wgs84(first: float, second: float) -> dict:
    from flights.models import Flight
    from flights.utils.excel_settlement_report import resolve_north_east_meters, sk42_meters_to_wgs84

    north, east = resolve_north_east_meters(first, second)
    normalized = Flight.normalize_coordinates_field(f'{north} {east}')
    if normalized:
    ...
    if not _in_ukraine_bbox(lat, lon):
        lat_swap, lon_swap = sk42_meters_to_wgs84(east, north)
        if lat_swap is not None and lon_swap is not None and _in_ukraine_bbox(lat_swap, lon_swap):
            north, east, lat, lon = east, north, lat_swap, lon_swap
        else:
            logger.warning(
                'TG-координаты вне зоны Украины: north=%s east=%s lat=%s lon=%s',
                north,
                east,
                lat,
                lon,
    ...
    return {
        'coordinates_sk42': f'{int(round(north))} {int(round(east))}',
        'lat_wgs84': lat,
        'lon_wgs84': lon,
    }


def parse_report_coordinates(text: str) -> dict:
    """Координаты X/Y или «Координаты: N M» из подписи отчёта → WGS-84."""
    raw = text or ''
    x_match = _SK42_X_RE.search(raw)
    y_match = _SK42_Y_RE.search(raw)
    if x_match and y_match:
        return _sk42_pair_to_wgs84(float(x_match.group(1)), float(y_match.group(1)))

    coords_line = _COORDS_LINE_RE.search(raw)
    if coords_line:
        return _sk42_pair_to_wgs84(
            float(coords_line.group(1)),
            float(coords_line.group(2)),
        )

    vals = [int(match.group(1)) for match in _SK42_VALUE_RE.finditer(raw)]
    if len(vals) >= 2:
        return _sk42_pair_to_wgs84(float(vals[0]), float(vals[1]))
    return {}
...
def parse_telegram_report_message(text: str, *, pilot_callsign: str = '') -> dict:
    ...
    if match:
        ...
        td_match = _FLIGHT_NUM_TD_RE.search(raw)
        if td_match:
            flight_number = int(td_match.group(1))
        else:
    ...
    pilot_match = _PILOT_RE.search(raw)
    operator_match = _OPERATOR_RE.search(raw)
    callsign = (
        callsign_match.group(1).strip()
        if callsign_match
        else (
            pilot_match.group(1).strip()
            if pilot_match
            else (operator_match.group(1).strip() if operator_match else pilot_callsign)
        )
    )[:255]
    ...
    coords = parse_report_coordinates(raw)
...
def record_telegram_flight_report(
    *,
    ...
    if not coordinates_sk42 and raw_text:
        coords = parse_report_coordinates(raw_text)
        coordinates_sk42 = coords.get('coordinates_sk42', coordinates_sk42)
        lat_wgs84 = coords.get('lat_wgs84', lat_wgs84)
        lon_wgs84 = coords.get('lon_wgs84', lon_wgs84)
    ...
    defaults = {
        ...
        'target_type': (target_type or '')[:255],
        'video_mime': (video_mime or '')[:127],
        'video_size': video_size,
        'video_duration': video_duration,
    }
    if telegram_file_id:
        defaults['telegram_file_id'] = telegram_file_id[:255]
    if lat_wgs84 is not None and lon_wgs84 is not None:
        defaults['coordinates_sk42'] = (coordinates_sk42 or '')[:64]
        defaults['lat_wgs84'] = lat_wgs84
        defaults['lon_wgs84'] = lon_wgs84
    ...
    if download_video and telegram_file_id:
        from flights.utils.telegram_report_video import schedule_report_video_download

        schedule_report_video_download(report.id)
    ...
    return {
        ...
        'created': created,
        'is_successful': defeated,
        'has_video': bool(telegram_file_id),
        'has_coordinates': lat_wgs84 is not None and lon_wgs84 is not None,
    }
